...
- `src/assets/ocr-engine.js` 初始化 `oar-ocr-wasm`，通过 `onnxruntime-web` 从
  `/models/:file` 加载 ONNX session，缓存 OCR 模型实例，并提供预测方法。
- `src/assets/ocr.js` 驱动 OCR 页面 UI，包括文件选择、按需模型加载、OCR
  执行、结果渲染和复制文本。
...
部署配置已经写入 `deno.json` 的 `deploy` 命名空间，因此默认的 `deployctl deploy`
命令不会再猜测项目名。所需的 Wasm 文件已经包含在 `public/pkg` 目录内；PP-OCRv6
模型会通过 `/models/:file` 从 ModelScope 按需获取。
